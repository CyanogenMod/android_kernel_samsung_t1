...
static int sec_bat_check_cable_callback(void)
{
	...
	pr_info("%s: cable is : %d, online(0x%02x)", __func__,
		cable, value.intval);

	current_cable_type = vbus && (value.intval != 0x08) &&
			cable != CABLE_TYPE_NONE_MUIC &&
	...
	if (psy) {
		...
		if (value.intval == POWER_SUPPLY_HEALTH_OVERVOLTAGE &&
				old_health != value.intval) {
			old_health = value.intval;
			psy_bat->set_property(psy_bat,
					POWER_SUPPLY_PROP_HEALTH, &value);
			power_supply_changed(psy_bat);
		} else if (value.intval == POWER_SUPPLY_HEALTH_GOOD &&
				old_health != value.intval) {
			old_health = value.intval;
			psy_bat->set_property(psy_bat,
					POWER_SUPPLY_PROP_HEALTH, &value);
