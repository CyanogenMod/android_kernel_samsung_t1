...
static const struct lms501kf07_sequence_entry gokey_lcd_seq_display_on[] = {
	...
	{
	 .cmd = mdnie_ui_reg,
	 .cmd_len = ARRAY_SIZE(mdnie_ui_reg),
	 .msleep = 5,
	 },
	{
...
static void set_mdnie_data(struct lms501kf07_sequence_entry *init_seq, u8 *data)
{
	/* mdnie is on the 4th place in init sequence */
	init_seq[4].cmd = data;
}
...
void __init omap4_gokey_display_init(void)
{
	...
	if (system_rev >= 2) {
		...
		panel->seq_display_on_size =
				ARRAY_SIZE(gokey_lcd_seq_display_on_rev2);
	}
